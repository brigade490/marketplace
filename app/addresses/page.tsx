      ...
      <div className="max-w-2xl mx-auto">
        ...
        {addresses.length === 0 && !showForm && (
          <div className="bg-white text-center py-16" style={{ borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            ...
            <button onClick={openAdd} className="px-6 py-3 text-sm font-bold text-white" style={{ background: '#000', borderRadius: '999px' }}>
              Add Address
            </button>
          </div>
        )}
        ...
        <div className="space-y-4">
          {addresses.map(addr => (
            <div key={addr.id} className="bg-white" style={{ borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: '20px' }}>
        ...
        {showForm && (
          ...
          >
            <div className="bg-white w-full" style={{ borderRadius: '16px', padding: '32px', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-3 text-sm font-semibold" style={{ border: '1.5px solid #e5e7eb', borderRadius: '999px' }}>Cancel</button>
                  <button type="submit" disabled={saving} className="flex-1 py-3 text-sm font-bold text-white" style={{ background: '#000000', borderRadius: '999px', opacity: saving ? 0.6 : 1 }}>
                    {saving ? 'Saving...' : 'Save'}
                  </button>
                </div>
              </form>
