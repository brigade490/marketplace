...
export default function SellerAuthPage() {
  ...
  async function redirectAfterLogin(userId: string, supabase: ReturnType<typeof createClient>) {
    const { data } = await supabase.from('users').select('onboarding_completed, role').eq('id', userId).single();
  ...
  async function handleEmailContinue(e: React.FormEvent) {
    ...
    setLoading(false);
    if (existing) go('login-found');
    else go('signup-password');
  }
  ...
  if (view === 'choose') return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: 'var(--bg)' }}>
      <div className="w-full" style={card}>
        ...
        <p className="text-center text-sm mb-2" style={{ color: 'var(--text-inactive)' }}>Seller Portal</p>
        <p className="text-center text-xs mb-10" style={{ color: 'var(--text-inactive)' }}>Sell your products to thousands of B2B buyers</p>
        ...
        <p className="text-xs text-center mt-6" style={{ color: 'var(--text-inactive)' }}>
          Looking to buy?{' '}<a href="/auth/buyer" className="underline font-semibold" style={{ color: 'var(--text-primary)' }}>Buyer login</a>
        </p>
      </div>
    </div>
  );
  ...
  if (view === 'login') return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: 'var(--bg)' }}>
      <div className="w-full" style={card}>
        ...
        <h2 className="text-xl font-black mb-1" style={{ color: 'var(--text-primary)' }}>Seller login</h2>
        <p className="text-sm mb-6" style={{ color: 'var(--text-inactive)' }}>Sign in to manage your store.</p>
  ...
  if (view === 'signup-email') return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: 'var(--bg)' }}>
      <div className="w-full" style={card}>
        <a href="/" className="block text-center text-2xl font-black mb-8" style={{ color: 'var(--text-primary)' }}>Karobarrr</a>
        <h2 className="text-xl font-black mb-1" style={{ color: 'var(--text-primary)' }}>Create seller account</h2>
